					...
					if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - wheel1) < 1.5 and w1 then
						...
						Citizen.CreateThread(
							function()
								...
								TaskPlayAnim(ped, 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', 'machinic_loop_mechandplayer', 8.0, -8.0, -1, 1, 0
									, false, false, false)
				...
				ESX.TriggerServerCallback('core_vehicle:getIfVehicleOwned', function(vehOwned)
					...
					if dst < Config.DetectDistance and vehOwned then
						if localVehLockStatus == 1 or localVehLockStatus == 0 then
							...
							local findObj = GetClosestObjectOfType(vehCoords.x, vehCoords.y, vehCoords.z, 1.0, GetHashKey("prop_carjack"),
								false, false, false)
	...
	function(data)
		...
		if defaultParts[partType] then
			...
			if defaultParts[partType].type ~= currentVehicleParts[partType].type then
				TriggerServerEvent('core_vehicle:removePart', currentVehicleParts[partType].type,
					currentVehicleParts[partType].health)
			end
