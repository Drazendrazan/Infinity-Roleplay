    ...
    while true do
        ...
        if isReady then
            ...
            for k, v in pairs(liftData) do
                if Vdist(pedCoords, v.controldown.x, v.controldown.y, v.controldown.z) < 30.0 or
                    Vdist(pedCoords, v.controlup.x, v.controlup.y, v.controlup.z) < 30.0 then
                    if not v.isInUse then
                        ...
                        if not ExistElevator then
                            -- print("exist pas")
                            if not v.isUp then
                                localElevateId = GetClosestObjectOfType(v.down.x, v.down.y, v.down.z, 3.0,
                                    GetHashKey(v.elevateProps), false, false, false)
                            else
                                localElevateId = GetClosestObjectOfType(v.up.x, v.up.y, v.up.z, 3.0,
                                    GetHashKey(v.elevateProps), false, false, false)
                            end
                        ...
                        if not downDoorIsOpen then
                            if not ExistDownDoor then
                                localDownDoor = GetClosestObjectOfType(v.doorDownCoords.x, v.doorDownCoords.y,
                                    v.doorDownCoords.z, 1.0, GetHashKey(v.doorDown), false, false, false)
                            ...
                            if not ExistDownDoor then
                                localDownDoor = GetClosestObjectOfType(v.doorDownOpenCoords.x, v.doorDownOpenCoords.y,
                                    v.doorDownOpenCoords.z, 1.0, GetHashKey(v.doorDown), false, false, false)
                                if localDownDoor ~= 0 then
                                    v.doorDownID = localDownDoor
                                end
                            end
                        ...
                        if not upDoorIsOpen then
                            if not ExistUpDoor then
                                localUpDoor = GetClosestObjectOfType(v.doorUpCoords.x, v.doorUpCoords.y, v.doorUpCoords.z
                                    , 1.0, GetHashKey(v.doorUp), false, false, false)
                            ...
                            if not ExistUpDoor then
                                localUpDoor = GetClosestObjectOfType(v.doorUpOpenCoords.x, v.doorUpOpenCoords.y,
                                    v.doorUpOpenCoords.z, 1.0, GetHashKey(v.doorUp), false, false, false)
                                if localUpDoor ~= 0 then
                                    v.doorUpID = localUpDoor
                                end
                            end
                        ...
                        if v.elevateID == 0 then
                            ...
                            if not v.isUp then
                                ...
                                v.elevateID = CreateObject(tempmodel, v.down.x, v.down.y, v.offsetdown.z, false, false,
                                    true)
                            else
                        ...
                        if v.doorDownID == 0 then
                            ...
                            if v.downDoorIsOpen then
                                v.doorDownID = CreateObject(tempmodel, v.doorDownOpenCoords.x, v.doorDownOpenCoords.y,
                                    v.doorDownOpenCoords.z, false, false, true)
                            else
                                v.doorDownID = CreateObject(tempmodel, v.doorDownCoords.x, v.doorDownCoords.y,
                                    v.doorDownCoords.z, false, false, true)
                            end
                        ...
                        if v.doorUpID == 0 then
                            ...
                            if v.upDoorIsOpen then
                                v.doorUpID = CreateObject(tempmodel, v.doorUpOpenCoords.x, v.doorUpOpenCoords.y,
                                    v.doorUpOpenCoords.z, false, false, true)
                            else
                                v.doorUpID = CreateObject(tempmodel, v.doorUpCoords.x, v.doorUpCoords.y, v.doorUpCoords.z
                                    , false, false, true)
                            end
                        ...
                        if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic' then
                            if Vdist(pedCoords, v.controldown.x, v.controldown.y, v.controldown.z) < 1.0 then
                                if (incircle == false) then
                                    ...
                                    if not showncall then
                                        exports["okokTextUI"]:Open("Pressione [E] para chamar o elevador.", 'darkblue',
                                            'right')
                                        showncall = true
                                    end
                                end
                                ...
                                if IsControlJustReleased(1, 51) then
                                    exports['mythic_progbar']:Progress({
                                        name = "chamarelevador1",
                                        duration = 1000,
                                        label = "A chamar o elevador...",
                                        useWhileDead = false,
                                        canCancel = false,
                                        controlDisables = {},
                                        prop = {},
                            ...
                            elseif Vdist(pedCoords, v.controlup.x, v.controlup.y, v.controlup.z) < 1.0 then
                                ...
                                --    exports["okokTextUI"]:Close()
                                --else
                                --    exports["okokTextUI"]:Open("Pressione [E] para chamar o elevador.", 'darkblue', 'right')
                                --end
                                incircle = true
                                if IsControlJustReleased(1, 51) then
                                    exports['mythic_progbar']:Progress({
                                        name = "chamarelevador2",
                                        duration = 1000,
                                        label = "A chamar o elevador...",
                                        useWhileDead = false,
                                        canCancel = false,
                            ...
                            elseif Vdist(pedCoords, v.controlCabineDown.x, v.controlCabineDown.y, v.controlCabineDown.z) <
                                0.7 and not v.isInUse then
                                if (incircle == false) then
                                    --DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour appeler l'ascenseur.")
                                    if not shown then
                                        exports["okokTextUI"]:Open("Pressione [E] para subir.", 'darkblue', 'right')
                                        shown = true
                                    end
                                end
                                --if (incircle == false) then
                            ...
                            elseif Vdist(pedCoords, v.controlCabineUp.x, v.controlCabineUp.y, v.controlCabineUp.z) < 0.7 and
                                not v.isInUse then
                                --if (incircle == false) then
                                --    --DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour descendre.")
                                --    exports["okokTextUI"]:Open("Pressione [E] para descer.", 'darkblue', 'right')
                                --else
                                --    exports["okokTextUI"]:Close()
                                --end
                                if (incircle == false) then
...
AddEventHandler('bennylift:openDoorDown', function(code)
    ...
    if not liftData[code].downDoorIsOpen then
        ...
        SetEntityCoords(Door, liftData[code].doorDownOpenCoords.x, liftData[code].doorDownOpenCoords.y,
            liftData[code].doorDownOpenCoords.z)
...
AddEventHandler('bennylift:closeDoorDown', function(code)
    ...
    if liftData[code].downDoorIsOpen then
        ...
        SetEntityCoords(Door, liftData[code].doorDownCoords.x, liftData[code].doorDownCoords.y,
            liftData[code].doorDownCoords.z)
...
AddEventHandler('bennylift:closeDoorDownIfnotBusy', function(code)
    ...
    if not liftData[code].isInUse then
        ...
        SetEntityCoords(leftDoor, liftData[code].doorDownCoords.x, liftData[code].doorDownCoords.y,
            liftData[code].doorDownCoords.z)
        -- SetEntityCoords(rightDoor,liftData[code].doorDownRCoords.x,liftData[code].doorDownRCoords.y,liftData[code].doorDownRCoords.z)
    end
end)
...
AddEventHandler('bennylift:openDoorUp', function(code)
    ...
    if not liftData[code].upDoorIsOpen then
        ...
        SetEntityCoords(leftDoor, liftData[code].doorUpOpenCoords.x, liftData[code].doorUpOpenCoords.y,
            liftData[code].doorUpOpenCoords.z)
...
AddEventHandler('bennylift:closeDoorUp', function(code)
    ...
    if liftData[code].upDoorIsOpen then
        ...
        SetEntityCoords(leftDoor, liftData[code].doorUpCoords.x, liftData[code].doorUpCoords.y,
            liftData[code].doorUpCoords.z)
...
AddEventHandler('bennylift:FreezeVeh', function(veh, liftcode)
    ...
    AttachEntityToEntity(curVeh, liftData[liftcode].elevateID, 0, 0, distAttach, coordsToAttach.z, 0, 0,
        heading - liftData[liftcode].elevateHeading, 0, false, true, false, 2, true)
