...
TryToGetMedicalTreatment = function(medicIndex)
    ...
    ESX.TriggerServerCallback("chames_ambulance_medic:requiredAmount", function(required)
        if required then
            ...
            ESX.ShowNotification("HOSPITAL", "Há S.E.M em serviço", 2000, "warning")
        end
    end)
end
...
GetMedicalTreatment = function(medicIndex)
    ...
    PlayAnimation(Ped, "amb@world_human_bum_slumped@male@laying_on_right_side@base", "base", {
        flag = 9
    })

    AttachEntityToEntity(Ped, pedHandle, GetPedBoneIndex(pedHandle, 57005), -0.32, -0.6, -0.35, 240.0, 35.0, 149.0, true, true, false, true, 1, true)
...
BedThread = function(medicIndex, bed, bedHandle)
    ...
    SetEntityCoords(Ped, bed.Location - vector3(0.0, 0.0, 0.985))
    SetEntityHeading(Ped, bed.Heading)

    Citizen.Wait(500)

    PlayAnimation(Ped, "dead", "dead_a", {
        flag = 1
    })

    while GetGameTimer() - InBed < Default.HelpTime do
        Citizen.Wait(0)

        DisableAllControlActions(0)

        local timePercent = math.floor((GetGameTimer() - InBed) / Default.HelpTime * 100)
    ...
    SetEntityCoordsNoOffset(Ped, standingLocation, false, false, false, true)
    SetEntityHeading(Ped, standingHeading - 180.0)
    ...
    ESX.ShowNotification("HOSPITAL", "Foste tratado pelo Sr. Mário", 2000, "success")
end

TreatPlayer = function()
    StopScreenEffect("DeathFailOut")
    NetworkResurrectLocalPlayer(GetEntityCoords(Ped) - vector3(0.0, 0.0, 0.985), 180.0, true, false)
    SetPlayerInvincible(PlayerId(), false)
