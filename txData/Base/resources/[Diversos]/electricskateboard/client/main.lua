...
RegisterNetEvent('longboard:use')
AddEventHandler('longboard:use', function()
    RCCar.Start()
end)

RegisterNetEvent('longboard:pickup')
AddEventHandler('longboard:pickup', function()
    RCCar.Clear()
end)
...
RCCar.HandleKeys = function(distanceCheck)
	...
	if distanceCheck < Config.LoseConnectionDistance then
		...
		if IsControlPressed(0, 22) and Attached then
			...
			if IsControlJustReleased(0, 32) or IsControlJustReleased(0, 33) and not overSpeed then -- ARROW UP
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 6, 2500)
			end

			if IsControlPressed(0, 33) and not IsControlPressed(0, 32) and not overSpeed then -- ARROW DOWN
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 22, 1)
			end

			if IsControlPressed(0, 34) and IsControlPressed(0, 33) and not overSpeed then -- ARROW LEFT + DOWN
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 13, 1)
			end

			if IsControlPressed(0, 35) and IsControlPressed(0, 33) and not overSpeed then -- ARROW RIGHT + DOWN
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 14, 1)
			end

			if IsControlPressed(0, 32) and IsControlPressed(0, 33) and not overSpeed then -- ARROW UP + DOWN
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 30, 100)
			end

			if IsControlPressed(0, 34) and IsControlPressed(0, 32) and not overSpeed then -- ARROW LEFT + UP
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 7, 1)
			end

			if IsControlPressed(0, 35) and IsControlPressed(0, 32) and not overSpeed then -- ARROW RIGHT + UP
