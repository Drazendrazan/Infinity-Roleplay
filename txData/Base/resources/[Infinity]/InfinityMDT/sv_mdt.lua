    ...
    if xPlayer.job.name == 'police' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			TriggerClientEvent('InfinityMDT:toggleVisibilty', usource, reports, warrants, officer)
    		end)
    	end)
    end
end)

RegisterServerEvent("InfinityMDT:hotKeyOpen")
AddEventHandler("InfinityMDT:hotKeyOpen", function()
	local usource = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == 'police' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
...
RegisterServerEvent("InfinityMDT:getOffensesAndOfficer")
AddEventHandler("InfinityMDT:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		TriggerClientEvent("InfinityMDT:returnOffensesAndOfficer", usource, charges, officer)
	end)
end)

RegisterServerEvent("InfinityMDT:performOffenderSearch")
AddEventHandler("InfinityMDT:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.query("SELECT * FROM `users` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("InfinityMDT:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("InfinityMDT:getOffenderDetails")
AddEventHandler("InfinityMDT:getOffenderDetails", function(offender)
	local usource = source
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		MySQL.Async.fetchAll('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
			...
			MySQL.Async.fetchAll('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
				...
				TriggerClientEvent("InfinityMDT:returnOffenderDetails", usource, offender)
			end)
		end)
	end)
end)

RegisterServerEvent("InfinityMDT:getOffenderDetailsById")
AddEventHandler("InfinityMDT:getOffenderDetailsById", function(char_id)
	local usource = source
	print(char_id)
...
RegisterServerEvent("InfinityMDT:saveOffenderChanges")
AddEventHandler("InfinityMDT:saveOffenderChanges", function(id, changes, identifier)
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		['@id']  = id
	}, function(result)
		if result[1] then
...
RegisterServerEvent("InfinityMDT:saveReportChanges")
AddEventHandler("InfinityMDT:saveReportChanges", function(data)
...
RegisterServerEvent("InfinityMDT:deleteReport")
AddEventHandler("InfinityMDT:deleteReport", function(id)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
end)

RegisterServerEvent("InfinityMDT:submitNewReport")
AddEventHandler("InfinityMDT:submitNewReport", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerEvent("InfinityMDT:getReportDetailsById", id, usource)
	end)
...
RegisterServerEvent("InfinityMDT:sentencePlayer")
AddEventHandler("InfinityMDT:sentencePlayer", function(jailtime, charges, char_id, fine, players)
	local usource = source
	local jailmsg = ""
		...
		if src ~= 0 and GetPlayerName(src) then
			MySQL.Async.fetchAll('SELECT * FROM `characters` WHERE `identifier` = @identifier', {
				...
				if result[1].identifier == char_id then
					...
					if fine > 0 then
						TriggerClientEvent("InfinityMDT:billPlayer", usource, src, 'society_police', 'Fine: '..jailmsg, fine)
					end
					return
				end
			end)
...
RegisterServerEvent("InfinityMDT:performReportSearch")
AddEventHandler("InfinityMDT:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("InfinityMDT:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("InfinityMDT:performVehicleSearch")
AddEventHandler("InfinityMDT:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("InfinityMDT:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("InfinityMDT:performVehicleSearchInFront")
AddEventHandler("InfinityMDT:performVehicleSearchInFront", function(query)
	local usource = source
    ...
    if xPlayer.job.name == 'police' then
		MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE `plate` = @query", {
			...
			TriggerClientEvent("InfinityMDT:toggleVisibilty", usource)
			TriggerClientEvent("InfinityMDT:returnVehicleSearchInFront", usource, result, query)
		end)
	end
end)

RegisterServerEvent("InfinityMDT:getVehicle")
AddEventHandler("InfinityMDT:getVehicle", function(vehicle)
	local usource = source
	MySQL.query("SELECT * FROM `users` WHERE `identifier` = @query", {
		['@query'] = vehicle.owner
	}, function(result)
		if result[1] then
		...
		TriggerClientEvent("InfinityMDT:returnVehicleDetails", usource, vehicle)
	end)
end)

RegisterServerEvent("InfinityMDT:getWarrants")
AddEventHandler("InfinityMDT:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("InfinityMDT:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("InfinityMDT:submitNewWarrant")
AddEventHandler("InfinityMDT:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("InfinityMDT:completedWarrantAction", usource)
	end)
end)

RegisterServerEvent("InfinityMDT:deleteWarrant")
AddEventHandler("InfinityMDT:deleteWarrant", function(id)
	local usource = source
...
RegisterServerEvent("InfinityMDT:getReportDetailsById")
AddEventHandler("InfinityMDT:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("InfinityMDT:returnReportDetails", usource, result[1])
		end
	end)
end)
