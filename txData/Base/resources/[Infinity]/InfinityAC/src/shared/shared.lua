...
if IS_SERVER then
	_G.RegisterServerCallback = function(args)
		...
		local eventData = RegisterNetEvent('Infinity__server_callback:'..eventName, function(packed, src, cb)
			local source = tonumber(source)
			if not source then
				cb( msgpack_pack_args( eventCallback(src, table_unpack(msgpack_unpack(packed)) ) ) )
			else
				TriggerClientEvent(('Infinity__client_callback_response:%s:%s'):format(eventName, source), source, msgpack_pack_args( eventCallback(source, table_unpack(msgpack_unpack(packed)) ) ))
			end
		end)
		return eventData
	end
	...
	_G.TriggerClientCallback = function(args)
		...
		if not args or not args.source or not tonumber(args.source) then return end
		if tonumber(args.source) >= 0 then
			...
			local eventData = RegisterNetEvent(('Infinity__callback_retval:%s:%s:%s'):format(args.source, args.eventName, ticket), function(packed)
			...
			TriggerClientEvent(('Infinity__client_callback:%s'):format(args.eventName), args.source, msgpack_pack(args.args or {}), ticket)
	...
	_G.TriggerServerCallback = function(args)
		...
		local eventCallback = args.callback
		local eventName = args.eventName
		TriggerEvent('Infinity__server_callback:'..eventName, msgpack_pack(args.args or {}), args.source,
		function(packed)
			if eventCallback and prom.state == PENDING then eventCallback( table_unpack(msgpack_unpack(packed)) ) end
			prom:resolve( table_unpack(msgpack_unpack(packed)) )
		end)
...
if not IS_SERVER then
	...
	_G.RegisterClientCallback = function(args)
		...
		local eventCallback = args.eventCallback
		local eventName = args.eventName
		local eventData = RegisterNetEvent('Infinity__client_callback:'..eventName, function(packed, ticket)
			if type(ticket) == 'function' then
				ticket( msgpack_pack_args( eventCallback( table_unpack(msgpack_unpack(packed)) ) ) )
			else
				TriggerServerEvent(('Infinity__callback_retval:%s:%s:%s'):format(SERVER_ID, eventName, ticket), msgpack_pack_args( eventCallback( table_unpack(msgpack_unpack(packed)) ) ))
			end
		end)
		return eventData
	end
	...
	_G.TriggerServerCallback = function(args)
		...
		local prom = promise.new()
		local eventCallback = args.callback
		local eventData = RegisterNetEvent(('Infinity__client_callback_response:%s:%s'):format(args.eventName, SERVER_ID),
		function(packed)
			if eventCallback and prom.state == PENDING then eventCallback( table_unpack(msgpack_unpack(packed)) ) end
			prom:resolve( table_unpack(msgpack_unpack(packed)) )
		end)
		TriggerServerEvent('Infinity__server_callback:'..args.eventName, msgpack_pack( args.args ))
		if args.timeout ~= nil and args.timedout then
			local timedout = args.timedout
			SetTimeout(args.timeout * 1000, function()
				if
	...
	_G.TriggerClientCallback = function(args)
		...
		local eventCallback = args.callback
		local eventName = args.eventName
		TriggerEvent('Infinity__client_callback:'..eventName, msgpack_pack(args.args or {}),
		function(packed)
			if eventCallback and prom.state == PENDING then eventCallback( table_unpack(msgpack_unpack(packed)) ) end
			prom:resolve( table_unpack(msgpack_unpack(packed)) )
		end)
