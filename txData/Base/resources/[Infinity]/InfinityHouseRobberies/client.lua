...
RegisterNetEvent('InfinityHouseRobberies:attempt')
AddEventHandler('InfinityHouseRobberies:attempt', function(lockpicks)
  -- SAFE
  ...
  if isRobbing and DoesEntityExist(safe) then
    ...
    if GetDistanceBetweenCoords(playerCoords, safepos.x, safepos.y, safepos.z, true) <= 3.0 then
      ...
      if math.random(1, 10) <= 3 then
        TriggerServerEvent('InfinityHouseRobberies:removeLockpick')
      end
    end
  end
  -- HOUSE
    ...
    ESX.TriggerServerCallback('Infinity:getJobsOnline', function(police)
      if police >= 2 then
        ...
        for id, v in pairs(robbableHouses) do
          ...
          if cops >= 0 then
            if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
              TriggerServerEvent("Infinity:log", id)
              nearHouse = true
              ...
              Skillbar.Start({
                ...
                Skillbar.Start({
                  ...
                  TriggerEvent('InfinityHouseRobberies:createHouse', id)
                  isLockpicking = false
                end, function()
                  isLockpicking = false
                  policeAlert(playerCoords, v.copJob)
        ...
        if not nearHouse then
          ESX.ShowNotification("Assalto a Casa", "Esta porta é reforçada", 2000, "error")
        end
      else
        ESX.ShowNotification("Assaltos a Casa", "Não existe policia suficiente", 2000, "error")
      end
    end)
  ...
  elseif not isNight() and not isRobbing then
    ESX.ShowNotification("Assaltos a Casa", "Está muito claro cá fora, dá muita cana", 2000, "error")
    --TriggerEvent('notification', 'Está muito claro cá fora, dá muita cana', 2)
  end
end)


RegisterNetEvent('InfinityHouseRobberies:policeattempt')
AddEventHandler('InfinityHouseRobberies:policeattempt', function()
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  for id, v in pairs(robbableHouses) do
    if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
      TriggerEvent('lockpickAnimation')
      exports['mythic_progbar']:Progress({
        ...
        isLockpicking = false
        if not status then
          TriggerEvent('InfinityHouseRobberies:policeenter', id)
        end
      end)
    end
  end
...
RegisterNetEvent('InfinityHouseRobberies:policeenter')
AddEventHandler('InfinityHouseRobberies:policeenter', function(id)
  ...
  while isRobbing do
    ...
    if GetDistanceBetweenCoords(playerCoords, house.x + 3.6, house.y - 15, house.z - 50, true) < 2.5 then
      ...
      if IsControlJustPressed(0, 38) then
        TriggerEvent('InfinityHouseRobberies:deleteHouse', id)
      end
    end
  end
end)

RegisterNetEvent('InfinityHouseRobberies:createHouse')
AddEventHandler('InfinityHouseRobberies:createHouse', function(id)
  local house = robbableHouses[id]
  ...
  if math.random(1, 10) < 2 then
    TriggerEvent('InfinityHouseRobberies:createDog')
  end
...
RegisterNetEvent('InfinityHouseRobberies:deleteHouse')
AddEventHandler('InfinityHouseRobberies:deleteHouse', function(id)
  local house = robbableHouses[id]
...
-- A FAZER
RegisterNetEvent('InfinityHouseRobberies:client:search')
AddEventHandler('InfinityHouseRobberies:client:search', function(id, table)
        ...
        for i = 1, #myRobbableItems do
          if (
            ...
            DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"],
              generator.z + myRobbableItems[i]["z"], 'Pressiona [~g~H~w~] para procurar ' .. myRobbableItems[i]["name"])
            if IsControlJustReleased(1, 74) then
              myRobbableItems[i]['isSearched'] = true
              TriggerServerEvent('InfinityHouseRobberies:server:search', isRobbing, myRobbableItems)
              ...
              exports['mythic_progbar']:Progress({
                ...
                if not status then
                  TriggerServerEvent('InfinityHouseRobberies:searchItem', isRobbing)
                end
              end)
            end
          end
...
function buildBasicHouse(generator)
  ...
  local mpdelta = CreateObject(GetHashKey("V_16_mid_hall_mesh_delta"), generator.x + 3.69693000, generator.y - 5.80020100
  ...
  local beddecal = CreateObject(GetHashKey("V_16_MID_bed_over_decal"), generator.x + 7.82861300, generator.y + 1.04696700
  ...
  local storage2 = CreateObject(GetHashKey("v_res_tre_storagebox"), generator.x + 9.75982700, generator.y - 1.35874100,
    generator.z + 1.29625800, false, false, false)
  local basketmess = CreateObject(GetHashKey("v_res_tre_basketmess"), generator.x + 8.70730600, generator.y - 2.55503600
...
function DeleteSpawnedHouse(id)
  ...
  repeat
    ...
    local distance = GetDistanceBetweenCoords(housePosition["x"], housePosition["y"], (housePosition["z"] - 24.0), pos,
      true)
...
AddEventHandler('InfinityHouseRobberies:createDog', function()
...
AddEventHandler('safe:success', function()
  ...
  safepos = { ["x"] = 0, ["y"] = 0, ["z"] = 0 }
  TriggerServerEvent('InfinityHouseRobberies:giveMoney')
end)
