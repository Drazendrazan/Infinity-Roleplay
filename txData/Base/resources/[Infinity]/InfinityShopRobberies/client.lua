...
RegisterNetEvent('InfinityShopRobberies:onPedDeath')
AddEventHandler('InfinityShopRobberies:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('InfinityShopRobberies:msgPolice')
AddEventHandler('InfinityShopRobberies:msgPolice', function(store, robber)
...
RegisterNetEvent('InfinityShopRobberies:removePickup')
AddEventHandler('InfinityShopRobberies:removePickup', function(bank)
    for i = 1, #objects do
...
RegisterNetEvent('InfinityShopRobberies:robberyOver')
AddEventHandler('InfinityShopRobberies:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('InfinityShopRobberies:talk')
AddEventHandler('InfinityShopRobberies:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('InfinityShopRobberies:rob')
AddEventHandler('InfinityShopRobberies:rob', function(i, ped)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('InfinityShopRobberies:pickUp', i)
                                DeleteObject(bag)
                                break
                            end
                        else
...
RegisterNetEvent('InfinityShopRobberies:resetStore')
AddEventHandler('InfinityShopRobberies:resetStore', function(i)
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('InfinityShopRobberies:pedDead', i)
                    break
                end
            end
            Wait(5000)
                ...
                for k, v in pairs(peds) do
                    ...
                    if HasEntityClearLosToEntityInFront(me, v, 19) and not IsPedDeadOrDying(v) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(v), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            local cops = nil
                            ESX.TriggerServerCallback('Infinity:getJobsOnline', function(police) -- TA AQUI ANIMAL
                            ...
                            ESX.TriggerServerCallback('InfinityShopRobberies:canRob', function(cb)
                                canRob = cb
                            end, i, cops)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                TriggerServerEvent('InfinityShopRobberies:alarm', i)
                                robbing = true
                                Citizen.CreateThread(function()
                                    while robbing do
                                        Wait(10)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(v), true) <= 7.5 then
                                    if not IsPedDeadOrDying(v) then
                                        TriggerServerEvent('InfinityShopRobberies:rob', i, me)
                                        while robbing do
                                            Wait(0)
                                            if IsPedDeadOrDying(v) then
                                                robbing = false
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('InfinityShopRobberies:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                        near = true
