...
function Process()
	isProcessing = true
	TriggerServerEvent('InfinityFarmer:process')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), config.zones.process.coords, false) > 4 then
			TriggerServerEvent('InfinityFarmer:cancelProcessing')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject then
			...
			if IsControlJustReleased(0, 7) and not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR2" then
				ESX.TriggerServerCallback('InfinityFarmer:haveItem', function(haveItem)
					if haveItem then
						isPickingUp = true
				
							ESX.TriggerServerCallback('InfinityFarmer:canPickUp', function(canPickUp)
									...
									if canPickUp then												
										...
										TriggerServerEvent('InfinityFarmer:pickedUp')
									end
