...
SpawnLocalVehicle = function(data)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
        ESX.ShowNotification("Garagem", "Por favor, retire o veículo da estrada", 2000, 'info')
		return
	end
...
SpawnVehicle = function(data,recuperar)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
        ESX.ShowNotification("Garagem", 'Está um veículo muito próximo', 2000, 'info')
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                ESX.ShowNotification("Garagem", 'O teu carro já está na cidade', 2000, 'info')
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        Citizen.Wait(100)
        TriggerServerEvent('InfinityGarages:modifystate', vehicleProps, 0, nil)
        TriggerEvent('InfinityPhone:client:updateVehState', vehicleProps.plate, 'OUT', 0)
        HandleCamera(cachedData["currentGarage"])
    end)
    if recuperar then
        ESX.ShowNotification("Garagem", 'Pagaste 200$ para recuperar a tua viatura', 2000, 'success')
        TriggerServerEvent('InfinityGarages:pay')
    end
    ClearMenu()
end
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("InfinityGarages:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
                ESX.ShowNotification("Garagem", 'Este veículo não te pertence', 2000, 'error')
			end
...
SaveInGarage = function(garage)
    ...
    ESX.ShowNotification("Garagem", 'Veículo guardado '..garage, 2000, 'success')
    Citizen.Wait(500)

    deleteCar(vehicle)
    TriggerServerEvent('InfinityGarages:modifystate', vehicleProps, 0, garage)
    TriggerEvent('InfinityPhone:client:updateVehState', vehicleProps.plate, garage, 0)
    CloseMenu()
end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return ESX.ShowNotification("Garagem", 'Modelo inválido, cria um ticket', 2000, 'info')
    end
