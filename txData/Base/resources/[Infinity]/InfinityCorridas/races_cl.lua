...
RegisterCommand("corridas", function(source, args)
    if args[1] == "limpar" or args[1] == "sair" then
        ...
        if raceStatus.state == RACE_STATE_JOINED or raceStatus.state == RACE_STATE_RACING then
            cleanupRace()
            TriggerServerEvent('InfinityCorridas:leaveRace_sv', raceStatus.index)
        end
    ...
    elseif args[1] == "iniciar" then
        ...
        if amount then
            ...
            if #recordedCheckpoints > 0 then
                ...
                TriggerServerEvent('InfinityCorridas:createRace_sv', amount, startDelay, startCoords, recordedCheckpoints)
                flares(startCoords)
    ...
    elseif args[1] == "cancelar" then
        ...
        TriggerServerEvent('InfinityCorridas:cancelRace_sv')
    else
        return
    end
end)
...
RegisterNetEvent("InfinityCorridas:createRace_cl")
AddEventHandler("InfinityCorridas:createRace_cl", function(index, amount, startDelay, startCoords, checkpoints)
...
RegisterNetEvent("InfinityCorridas:loadRace_cl")
AddEventHandler("InfinityCorridas:loadRace_cl", function(checkpoints)
...
RegisterNetEvent("InfinityCorridas:joinedRace_cl")
AddEventHandler("InfinityCorridas:joinedRace_cl", function(index)
...
RegisterNetEvent("InfinityCorridas:removeRace_cl")
AddEventHandler("InfinityCorridas:removeRace_cl", function(index)
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            TriggerServerEvent('InfinityCorridas:finishedRace_sv', raceStatus.index, currentTime)
                    ...
                    if proximity < config_cl.joinProximity then
                        ...
                        if currentTime < race.startTime then
                            ...
                            if IsControlJustReleased(1, config_cl.joinKeybind) then
                                TriggerServerEvent('InfinityCorridas:joinRace_sv', index)
                                break
                            end
                        end
                    end
