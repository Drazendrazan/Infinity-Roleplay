	...
	while true do
		...
		if IsPedInAnyVehicle(ped, false) then
			if GetIsTaskActive(ped, 2) then
				...
				data.fuelLevel = GetVehicleFuelLevel(car)
				...
				--if int == 0 then
					...
					TriggerServerEvent('InfinityPhone:server:UpdateVehConfigBD', coords, heading, data, health)
				--end
				Wait(3000)
			end
			waitWi = 5
		...
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, x, y, z, false) < 20 then
			...
			for k, v in ipairs(vehArea) do
				...
				if GetVehicleNumberPlateText(v):gsub('%s+', '') == data.data.plate:gsub('%s+', '') then
					ESX.ShowNotification("TELEMÓVEL", 'O teu carro já foi entregue', 2000, 'error')
					canSpawn = false
					break
				end
			end
			...
			if canSpawn then
				...
				ESX.Game.SpawnVehicle(vehicle.model, {
					...
					if data.data.health and data.data.health < 900 then
					...
					if vehicle.bodyHealth < 900 then
						...
						SetVehicleDamage(callback_vehicle, 0.0, 1.0, 0.1,
							math.floor(1000 - tonumber(string.format("%.1f", vehicle.bodyHealth))) + 0.0, 1850.0, true) --800
						SetVehicleDamage(callback_vehicle, -0.2, 1.0, 0.5,
							math.floor(1000 - tonumber(string.format("%.1f", vehicle.bodyHealth))) + 0.0, 650.0, true) -- 50
						SetVehicleDamage(callback_vehicle, -0.7, -0.2, 0.3,
							math.floor(1000 - tonumber(string.format("%.1f", vehicle.bodyHealth))) + 0.0, 500.0, true) --00 50
					end
					SetVehicleFuelLevel(callback_vehicle, vehicle.fuelLevel)
				end)
			else
			end
		else
			ESX.ShowNotification("TELEMÓVEL", 'Tens de estar mais perto da localização do veículo', 2000, 'error')
		end
	else
		ESX.ShowNotification("TELEMÓVEL", 'Não podes pedir um veículo dentro de um veículo', 2000, 'error')
	end
end)


RegisterNetEvent('InfinityPhone:client:UpdateVehConfig')
AddEventHandler('InfinityPhone:client:UpdateVehConfig', function(coords, heading, vData)
	local data = {
		plate = vData.plate,
		vehicle = vData,
		health = health,
...
RegisterNetEvent('InfinityPhone:client:updateVehState')
AddEventHandler('InfinityPhone:client:updateVehState', function(plate, garage, state)
	local data = {
		plate = plate,
		garage = garage,
		state = state
...
RegisterNUICallback('GiveCarKey', function(data, cb)
	...
	if distance ~= -1 and distance <= 3.0 then
		TriggerServerEvent("InfinityPhone:server:giveCarKeys", GetPlayerServerId(player), data.plate)
		TriggerEvent('InfinityPhone:client:ActionCallback', 'GiveCarKey', true)
	else
		TriggerEvent('InfinityPhone:client:ActionCallback', 'GiveCarKey', false)
	end
end)

RegisterNUICallback('RemoveAllCarKey', function(data, cb)
	--actionCb['RemoveAllCarKey'] = cb
	TriggerServerEvent("InfinityPhone:server:RemoveAllCarKey", "RemoveAllCarKey", data.plate)
end)
