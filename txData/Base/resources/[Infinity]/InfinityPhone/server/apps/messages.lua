RegisterServerEvent('InfinityPhone:server:GetTexts')
AddEventHandler('InfinityPhone:server:GetTexts', function()
local src = source
...
local thingy = getNumberPhone(player.identifier)
    Citizen.CreateThread(function()
        MySQL.Async.fetchAll("SELECT * FROM phone_texts WHERE (sender = @number AND sender_deleted = 0) OR (receiver = @number AND receiver_deleted = 0)", { ['@number'] = thingy }
         , function(result)
            TriggerClientEvent('InfinityPhone:client:SetupData', src, {{ name = 'messages', data = result }})
         end)
    end)
end)

function getMessages(identifier)
    MySQL.query("SELECT * FROM phone_texts INNER JOIN users ON users.identifier = = WHERE phone_texts.receiver = users.phone_number OR phone_texts.sender = users.phone_number", {
         identifier
    }, function(result)
        return result
    end)
...
ESX.RegisterServerCallback('InfinityPhone:server:SendText', function(source, cb, data)
    local src = source
                ...
                if otherplayer ~= nil then
                    ...
                    function(contact)
                        if contact[1] ~= nil then
                            TriggerClientEvent('InfinityPhone:client:ReceiveText', tonumber(otherplayer), contact[1].name, data.message, thingy.phone_number)
                        else
                            TriggerClientEvent('InfinityPhone:client:ReceiveText', tonumber(otherplayer), thingy.phone_number, data.message, thingy.phone_number)
                        end
                    end)
                end
                cb(data)
...
RegisterServerEvent('InfinityPhone:server:SendTextbot')
AddEventHandler('InfinityPhone:server:SendTextbot', function(textmsg)
local src = source
local thingy = getIdentity(src)
        ...
        MySQL.Async.execute('INSERT INTO phone_texts (`sender`, `receiver`, `message`) VALUES(@sender, @receiver, @message)', { ['sender'] = "000000000", ['receiver'] = thingy.phone_number, ['message'] = textmsg }, function(status)
            if status > 0 then
                if textmsg ~= nil then
                    ...
                    TriggerClientEvent('InfinityPhone:client:ReceiveText', src, "000000000", textmsg)
                else
                    stat = (false)
                end
            else
...
ESX.RegisterServerCallback('InfinityPhone:server:DeleteConversation', function(source, cb, data)
    local src = source
    local thingy = getIdentity(src)
