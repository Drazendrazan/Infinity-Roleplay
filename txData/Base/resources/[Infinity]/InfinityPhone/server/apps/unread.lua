...
--     local src = source
--     local mPlayer = exports['InfinityPhone']:FetchComponent('Fetch'):Source(src)
--     if mPlayer ~= nil then
...
-- end)
--[[
RegisterServerEvent('InfinityPhone:server:Reds')
AddEventHandler('InfinityPhone:server:Reds', function()
    local src = source
        ...
        exports['ghmattimysql']:scalar('SELECT data FROM phone_unread WHERE identifier = @identifier', { ['identifier'] = thingy.identifier }, function(unread)
            if unread ~= nil then
                if json.decode(unread) ~= nil then
                    ...
                    print(unread)
                    TriggerClientEvent('InfinityPhone:client:SyncUnread', src, json.decode(unread))
                else
                    ...
                    print(Unreadeds)
                    TriggerClientEvent('InfinityPhone:client:SyncUnread', src, unreads)
                end
            else
                unreads = ResetUnreads()
                Unreadeds = {
...
RegisterServerEvent('InfinityPhone:server:phone-unread')
AddEventHandler('InfinityPhone:server:phone-unread', function(identifier, data)
    local src = source
    local thingy = getIdentity(src)
...
ESX.RegisterServerCallback('InfinityPhone:server:SetUnread', function(source, cb, data)
    local src = source
    local thingy = getIdentity(src)
        ...
        if unreads[data.app] ~= data.unread then
            ...
            TriggerEvent('InfinityPhone:server:phone-unread', thingy.identifier, unreads)
        end
