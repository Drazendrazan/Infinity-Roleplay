RegisterServerEvent('InfinityPhone:server:irc')
AddEventHandler('InfinityPhone:server:irc', function()
local src = source
    ...
    Citizen.CreateThread(function()
        local myChannels = {}
         exports['ghmattimysql']:execute('SELECT channel FROM phone_irc_channels WHERE identifier = @identifier', { ['identifier'] = player.identifier }, function(channels) 
            TriggerClientEvent('InfinityPhone:client:SetupData', src, { { name = 'irc-channels', data = channels } })
        end)
    end)
end)

ESX.RegisterServerCallback('InfinityPhone:server:IRCJoinChannel', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('InfinityPhone:server:IRCLeaveChannel', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('InfinityPhone:server:IRCGetMessages', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('InfinityPhone:server:IRCNewMessage', function(source, cb, data)
    local src = source
    local player = ESX.GetPlayerFromId(source)
                        ...
                        exports['ghmattimysql']:execute('SELECT * FROM phone_irc_channels WHERE channel = @channel', { ['channel'] = data.channel }, function(data2)
                            for k, v in ipairs(data2) do
                                    ...
                                    local tPlayer = v.identifier
                                        TriggerClientEvent('InfinityPhone:client:ReceiveNewIRCMessage', otherplayer, { channel = data.channel, message = data.message })
                            end
                        end)
                    end)
                end
