                        ...
                        if IsControlJustPressed(0, 74) then
                            ESX.TriggerServerCallback('Infinity:getJobsOnline', function(police)
                                if v.copJob == 'police' then
                                    TriggerServerEvent('InfinityAmmuRob:start', police, k)
                                end
                                draw = false
                            end)
                            Citizen.Wait(500)
...
--[[ RegisterNetEvent('InfinityAmmuRob:reset')
AddEventHandler('InfinityAmmuRob:reset', function(store)
    local Store = store
...
RegisterNetEvent('InfinityAmmuRob:start')
AddEventHandler('InfinityAmmuRob:start', function(store)
    robbing = true
    local playerCoords = GetEntityCoords(PlayerPedId())
    DecorSetInt(GetPlayerPed(-1), "IsOutlaw", 2)
    TriggerServerEvent("esx_outlawalert:Vangelico",
        ...
        while robbing do
                ...
                if not Spot.broke then
                    if (GetDistanceBetweenCoords(plyCoords, Spot.coord[1], Spot.coord[2], Spot.coord[3], true) < 1.0 ) then
                        ...
                        if IsControlJustPressed(0, 74) then
                            ...
                            TriggerEvent("esx_status:add", 10000, "stress")
                            if breakchance <= hasweapon.chance then
                                ...
                                TriggerServerEvent('InfinityAmmuRob:giveItem')
                                Citizen.Wait(500)
                                TriggerServerEvent('InfinityAmmuRob:giveItem')
                                PlaySound(-1, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            else
                                TriggerEvent("esx_status:add", 40000, "stress")
                                Citizen.Wait(2100)
                                ClearPedTasksImmediately(player)
                            end
                        end
            ...
            if not Config.AmmuStores[store]['box'].open then
                if (GetDistanceBetweenCoords(plyCoords, Config.AmmuStores[store]['box'].coord[1], Config.AmmuStores[store]['box'].coord[2], Config.AmmuStores[store]['box'].coord[3], true) < 1.0 ) then
                    ...
                    if IsControlJustPressed(0, 74) then
                        TriggerEvent("esx_status:add", 55000, "stress")
                        animCaixa()
...
function animCaixa()
    ...
    TriggerServerEvent("InfinityAmmuRob:giveMoney")
    Citizen.Wait(2500)
end
