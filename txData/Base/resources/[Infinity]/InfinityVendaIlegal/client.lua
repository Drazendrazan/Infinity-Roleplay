...
function givePackageAnim(item, count)
    ...
    print(item)
    TriggerServerEvent('InfinityVendaIlegal:removeitem', item, count)
...
function giveMoneyAnimPed(ped, price)
    ...
    Wait(4000)
    TriggerServerEvent("InfinityVendaIlegal:addmoney", price)
                        ...
                        if GetDistanceBetweenCoords(coords, shopsP[k].x, shopsP[k].y, shopsP[k].z, false) < 2.0 then
                            if (IsControlPressed(1, 38)) then
                                ...
                                if hour > 0 and hour < 24 then
                                    ESX.TriggerServerCallback('InfinityVendaIlegal:getItemsToSell', function (cb)
                                        if cb ~= false then
                                            print(cb.name)
                                            print(cb.price)
                                            print(cb.count)
                                            ESX.ShowNotification("Pawn Shop", "Não posso comprar esses produtos. Vou ver se consigo alguém para te comprar isso, fica atento e boa sorte!", 5000, 'info')
                                            TriggerEvent("InfinityVendaIlegal:startSellingItems")
                                        else
                                            ESX.ShowNotification("Pawn Shop", "Não tenho nada para comprar, volte mais tarde!", 2000, 'error')
                                        end
                                    end)
                                    Wait(2500)
                                else
                                    ESX.ShowNotification("Pawn Shop", "Loja Fechada", 2000, 'error')
                                    Wait(2500)
                               end
                            end
                        end
...
RegisterNetEvent('InfinityVendaIlegal:startSellingItems')
AddEventHandler('InfinityVendaIlegal:startSellingItems', function()
    local wait = 2000 --math.random(100000,200000)
    Wait(wait)
    if not onSelling then
        ESX.TriggerServerCallback('InfinityVendaIlegal:getItemsToSell', function (cb)
            if cb ~= false then
                TriggerEvent("InfinityVendaIlegal:sellToRandomNPC")
            end
        end)
    end
end)

RegisterNetEvent('InfinityVendaIlegal:sellToRandomNPC')
AddEventHandler('InfinityVendaIlegal:sellToRandomNPC', function()
    onSelling = true
                    ...
                    ESX.TriggerServerCallback('InfinityVendaIlegal:getItemsToSell', function (cb)
                        if cb ~= false then
                         ...
                         PlayAmbientSpeech1(sellNPC, "GENERIC_BYE", "SPEECH_PARAMS_FORCE_SHOUTED_CRITICAL")
                            TriggerEvent("InfinityVendaIlegal:startSellingItems")
                            TriggerEvent("InfinityVendaIlegal:deleteNPCafterSell", sellNPCpos[num].x, sellNPCpos[num].y, sellNPCpos[num].z, sellNPC)
                            onSelling = false
                            RemoveBlip(blip)
                        else
                            ESX.ShowNotification("Pawn Shop", "Não tinhas o produto certo para vender!", 2000, 'error')
                            TriggerEvent("InfinityVendaIlegal:deleteNPCafterSell", sellNPCpos[num].x, sellNPCpos[num].y, sellNPCpos[num].z, sellNPC)
                            onSelling = false
                            RemoveBlip(blip)
                        end
...
RegisterNetEvent('InfinityVendaIlegal:deleteNPCafterSell')
AddEventHandler('InfinityVendaIlegal:deleteNPCafterSell', function(x,y,z,npc)
    CreateThread(function()
        while true do
            Wait(50)
            local coords = GetEntityCoords(GetPlayerPed(-1))
