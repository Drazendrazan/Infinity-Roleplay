...
ESX.RegisterServerCallback('vangelicoheist:server:checkTime', function(source, cb)
    ...
    if (os.time() - lastrob) < Config['VangelicoHeist']['nextRob'] and lastrob ~= 0 then
        ...
        lastrob = os.time()
        start = true
        discordLog('**Info:** iniciado assalto à joalheria', src)
        cb(true)
    end
end)
...
AddEventHandler('vangelicoheist:server:rewardItem', function(item)
    ...
    if player then
        ...
        if not whitelistItems[item] then
...
AddEventHandler('vangelicoheist:server:sellRewardItems', function()
    ...
    if player then
        ...
        discordLog('**Info:** ganhou €' .. totalMoney .. ' no assalto à joalheria', src)
...
function discordLog(message, id)
    exports["Pack"]:LogToDiscord(id, "https://discord.com/api/webhooks/1043622876366000280/vbs6fBBbSbJvtsJWo3Y1qsfoYLgmwejvnHExmmOhvyupEFZTqs3LHaM40QmjXL61_RRu", "Joalheria", message)
end