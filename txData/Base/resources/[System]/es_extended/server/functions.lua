    ...
    if not command.allowConsole and playerId == 0 then
      ...
      if error then
        if playerId == 0 then
          ...
          xPlayer.showNotification("ESX", error, 2000, "error")
        end
      else
        cb(xPlayer or false, args, function(msg)
          if playerId == 0 then
            ...
            xPlayer.showNotification("ESX", msg, 2000, "success")
          end
        end)
      end
    end
...
function Core.SavePlayers(cb)
  ...
  if count > 0 then
    ...
    MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ? WHERE `identifier` = ?"
      ...
      function(results)
        if results then
          if type(cb) == 'function' then cb() else print(('[^2INFO^7] Saved %s %s over %s ms'):format(count,
            count > 1 and 'players' or 'player', (os.time() - time) / 1000000)) end
        end
      end)
