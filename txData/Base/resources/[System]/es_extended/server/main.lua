...
AddEventHandler('chatMessage', function(playerId, author, message)
  ...
  if message:sub(1, 1) == '/' and playerId > 0 then
    ...
    --xPlayer.showNotification("CHAT", TranslateCap('commanderror_invalidcommand', commandName), 2000, 'error')
  end
end)
...
if not Config.OxInventory then
  ...
  AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
    ...
    if type == 'item_standard' then
      ...
      if itemCount > 0 and sourceItem.count >= itemCount then
        if targetXPlayer.canCarryItem(itemName, itemCount) then
          ...
          sourceXPlayer.showNotification("INVENTÁRIO",
            TranslateCap('gave_item', itemCount, sourceItem.label, targetXPlayer.name), 2000, 'info')
          targetXPlayer.showNotification("INVENTÁRIO",
            TranslateCap('received_item', itemCount, sourceItem.label, sourceXPlayer.name), 2000, 'info')
        else
          sourceXPlayer.showNotification("INVENTÁRIO", TranslateCap('ex_inv_lim', targetXPlayer.name), 2000, 'warning')
        end
      else
        sourceXPlayer.showNotification("INVENTÁRIO", TranslateCap('imp_invalid_quantity'), 2000, 'error')
      end
    elseif type == 'item_account' then
      if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
        ...
        sourceXPlayer.showNotification("INVENTÁRIO",
          TranslateCap('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName].label,
            targetXPlayer.name), 2000, 'info')
        targetXPlayer.showNotification("INVENTÁRIO",
          TranslateCap('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName].label,
            sourceXPlayer.name), 2000, 'info')
      else
        sourceXPlayer.showNotification("INVENTÁRIO", TranslateCap('imp_invalid_amount'), 2000, 'error')
      end
      ...
      if sourceXPlayer.hasWeapon(itemName) then
        ...
        if not targetXPlayer.hasWeapon(itemName) then
          ...
          if weaponObject.ammo and itemCount > 0 then
            ...
            sourceXPlayer.showNotification("INVENTÁRIO",
              TranslateCap('gave_weapon_withammo', weaponLabel, itemCount, ammoLabel, targetXPlayer.name), 2000, 'info')
            targetXPlayer.showNotification("INVENTÁRIO",
              TranslateCap('received_weapon_withammo', weaponLabel, itemCount, ammoLabel, sourceXPlayer.name), 2000,
              'info')
          else
            sourceXPlayer.showNotification("INVENTÁRIO", TranslateCap('gave_weapon', weaponLabel, targetXPlayer.name),
              2000, 'info')
            targetXPlayer.showNotification("INVENTÁRIO",
              TranslateCap('received_weapon', weaponLabel, sourceXPlayer.name), 2000, 'info')
          end
        else
          sourceXPlayer.showNotification("INVENTÁRIO",
            TranslateCap('gave_weapon_hasalready', targetXPlayer.name, weaponLabel), 2000, 'warning')
          targetXPlayer.showNotification("INVENTÁRIO",
            TranslateCap('received_weapon_hasalready', sourceXPlayer.name, weaponLabel), 2000, 'warning')
        end
      end
        ...
        if targetXPlayer.hasWeapon(itemName) then
          ...
          if weaponObject.ammo then
            ...
            if weapon.ammo >= itemCount then
              ...
              sourceXPlayer.showNotification("INVENTÁRIO",
                TranslateCap('gave_weapon_ammo', itemCount, ammoLabel, weapon.label, targetXPlayer.name), 2000, 'info')
              targetXPlayer.showNotification("INVENTÁRIO",
                TranslateCap('received_weapon_ammo', itemCount, ammoLabel, weapon.label, sourceXPlayer.name), 2000,
                'info')
            end
          end
        else
          sourceXPlayer.showNotification("INVENTÁRIO",
            TranslateCap('gave_weapon_noweapon', targetXPlayer.name, 2000, 'info'))
          targetXPlayer.showNotification("INVENTÁRIO",
            TranslateCap('received_weapon_noweapon', sourceXPlayer.name, weapon.label, 2000, 'info'))
        end
      end
    end
  end)
  ...
  AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount)
    ...
    if type == 'item_standard' then
      if itemCount == nil or itemCount < 1 then
        xPlayer.showNotification("INVENTÁRIO", TranslateCap('imp_invalid_quantity'), 2000, 'error')
      else
        ...
        if (itemCount > xItem.count or xItem.count < 1) then
          ...
          xPlayer.showNotification("INVENTÁRIO", TranslateCap('threw_standard', itemCount, xItem.label))
        end
      end
    elseif type == 'item_account' then
      if itemCount == nil or itemCount < 1 then
        xPlayer.showNotification("INVENTÁRIO", TranslateCap('imp_invalid_amount'), 2000, 'error')
      else
        ...
        if (itemCount > account.money or account.money < 1) then
          ...
          xPlayer.showNotification("INVENTÁRIO",
            TranslateCap('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(account.label)), 2000, 'info')
        end
      end
    elseif type == 'item_weapon' then
      ...
      if xPlayer.hasWeapon(itemName) then
        ...
        if weaponObject.ammo and weapon.ammo > 0 then
          ...
          xPlayer.showNotification("INVENTÁRIO", TranslateCap('threw_weapon_ammo', weapon.label, weapon.ammo, ammoLabel)
            , 2000, 'info')
        else
          pickupLabel = ('%s'):format(weapon.label)
          xPlayer.showNotification("INVENTÁRIO", TranslateCap('threw_weapon', weapon.label), 2000, 'info')
        end
  ...
  AddEventHandler('esx:useItem', function(itemName)
    ...
    if count > 0 then
      ...
      xPlayer.showNotification("INVENTÁRIO", TranslateCap('act_imp'), 2000, 'info')
    end
  end)
