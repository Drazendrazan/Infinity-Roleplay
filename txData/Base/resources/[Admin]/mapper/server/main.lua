...
function LoadMap(name)
	...
	local xml = LoadResourceFile('mapper', 'data/maps/' .. name .. '.xml')
...
function SaveMap(name, objects)
	...
	xml = xml .. '  <Metadata>'                                                                                             .. "\n"
	xml = xml .. '    <Creator>mapper</Creator>'                                                                         .. "\n"
	xml = xml .. '    <Name>mapper</Name>'                                                                               .. "\n"
	xml = xml .. '    <Description>mapper</Description>'                                                                 .. "\n"
	...
	local file = io.open('resources/[essential]/mapper/data/maps/' .. name .. '.xml', 'w')
...
RegisterServerEvent('mapper:beginSaveObjects')
AddEventHandler('mapper:beginSaveObjects', function(name)
	local _source = source
	ObjectsToSave[source .. '_' .. name] = {}
end)

RegisterServerEvent('mapper:endSaveObjects')
AddEventHandler('mapper:endSaveObjects', function(name)
	local _source = source
	SaveMap(name, ObjectsToSave[source .. '_' .. name])
	TriggerClientEvent('mapper:showNotification', _source, 'Map [' .. name .. '] saved')
end)


RegisterServerEvent('mapper:addObjectToSave')
AddEventHandler('mapper:addObjectToSave', function(name, object)
	local _source = source
	table.insert(ObjectsToSave[source .. '_' .. name], object)
end)

RegisterNetEvent('mapper:loadObjects')
AddEventHandler('mapper:loadObjects', function(name)
	
	local _source = source
	local dom     = LoadMap(name)

	if dom == nil then
		TriggerClientEvent('mapper:showNotification', _source, 'Map [' .. name .. '] does not exists')
	else
		...
		CreateThread(function()
			...
			for i=1, #objects, 1 do
				...
				TriggerClientEvent('mapper:loadObject', _source, objects[i])
			...
			TriggerClientEvent('mapper:showNotification', _source, 'Map [' .. name .. '] loaded')
