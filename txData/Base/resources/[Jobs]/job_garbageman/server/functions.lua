...
RegisterServerEvent('CORE_JOB_GARBAGEMAN:CheckIfItemExists_s') -- THIS EVENT CHECKS FOR ITEMS
AddEventHandler('CORE_JOB_GARBAGEMAN:CheckIfItemExists_s',function(player,item,itemamount,cb)
	if cfg.framework == "esx" then
...
RegisterServerEvent('CORE_JOB_GARBAGEMAN:GiveItem_s') -- THIS EVENT ADDS ITEMS
AddEventHandler('CORE_JOB_GARBAGEMAN:GiveItem_s',function(player,item,amount)
	if cfg.framework == "esx" then
		local xPlayer = Functions.GetPlayerFromId(player)
		if item == "cash" then
		...
		elseif item == "bank" then
			...
			if xPlayer.canCarryItem(item, amount) then
				...
				xPlayer.showNotification("RECICLADOR", "Não tens espaço suficiente no teu inventário", 2000, "error")
			end
		end
	end
	if cfg.framework == "qbcore" then
