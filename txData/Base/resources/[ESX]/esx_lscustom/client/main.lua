...
function OpenLSMenu(elems, menuName, menuTitle, parent)
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName, {
            ...
            if k == data.current.modType or isRimMod then
                ...
                if data.current.label == TranslateCap('by_default') or string.match(data.current.label, TranslateCap('installed')) then
                    ...
                    for i = 1, #Vehicles, 1 do
                        if GetEntityModel(vehicle) == joaat(Vehicles[i].vehicle_id) then
                            vehiclePrice = Vehicles[i].max_price
                            break
                        end
                    end
        ...
        menu.close()
        for k, v in pairs(myCar["windowsBroken"]) do
            myCar["windowsBroken"][k] = false
        end
        for k, v in pairs(myCar["doorsBroken"]) do
            myCar["doorsBroken"][k] = false
        end
        for k, v in pairs(myCar["tyreBurst"]) do
            myCar["tyreBurst"][k] = false
        end
        TriggerEvent('esx_lscustom:cancelInstallMod')
...
CreateThread(function()
    --for k, v in pairs(Config.Zones) do
        local blip = AddBlipForCoord(-205.1614, -1294.8611, 30.6256)
        ...
        if IsPedInAnyVehicle(playerPed, false) then
            ...
            if (ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic') or not Config.IsMechanicJobOnly then
                for k, v in pairs(Config.Zones) do
                    ...
                    if #(coords - zonePos) < 5.0 then
                        Near = true
                        Sleep = 0
                        if not lsMenuIsShowed then
                            if not HintDisplayed and not lsMenuIsShowed then
                                HintDisplayed = true
                                exports["okokTextUI"]:Open(v.Hint, "darkblue", "right")
                            end
