ESX.RegisterServerCallback('esx_documents:submitDocument', function(source, cb, data)
    ...
    MySQL.Async.insert("INSERT INTO user_documents (owner, data) VALUES (@owner, @data)",
        { ['@owner'] = xPlayer.identifier, ['@data'] = json.encode(data) }, function(id)
...
ESX.RegisterServerCallback('esx_documents:deleteDocument', function(source, cb, id)
    ...
    MySQL.Async.execute('DELETE FROM user_documents WHERE id = @id AND owner = @owner',
        ...
        if rowsChanged >= 1 then
            TriggerClientEvent('esx:showNotification', source, TranslateCap('document_deleted'))
            cb(true)
        else
            TriggerClientEvent('esx:showNotification', source, TranslateCap('document_delete_failed'))
            cb(false)
        end
    end)
end)
...
ESX.RegisterServerCallback('esx_documents:getPlayerDocuments', function(source, cb)
    ...
    local forms = {}
    if xPlayer ~= nil then
        MySQL.query("SELECT * FROM user_documents WHERE owner = ?", { xPlayer.identifier }, function(result)
...
ESX.RegisterServerCallback('esx_documents:getPlayerDetails', function(source, cb)
    ...
    MySQL.Async.fetchAll("SELECT firstname, lastname, dateofbirth FROM users WHERE identifier = @owner",
        { ['@owner'] = xPlayer.identifier }, function(result)
...
AddEventHandler('esx_documents:CopyToPlayer', function(targetID, aForm)
    ...
    MySQL.Async.insert("INSERT INTO user_documents (owner, data) VALUES (@owner, @data)",
        { ['@owner'] = targetxPlayer.identifier, ['@data'] = json.encode(aForm) }, function(id)
        if id ~= nil then
            MySQL.Async.fetchAll('SELECT * FROM user_documents where id = @id', { ['@id'] = id }, function(result)
                --print("Trying to dump: " .. dump(result))
                if (result[1] ~= nil) then
                    ...
                    TriggerClientEvent('esx:showNotification', _targetid, TranslateCap('copy_from_player'))
                    TriggerClientEvent('esx:showNotification', _source, TranslateCap('from_copied_player'))
                else
                    TriggerClientEvent('esx:showNotification', _source, TranslateCap('could_not_copy_form_player'))
                end
            end)
        else
            TriggerClientEvent('esx:showNotification', _source, TranslateCap('could_not_copy_form_player'))
