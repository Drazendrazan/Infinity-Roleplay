...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("esx_outlawalert:GetMyName")
end)
                            ...
                            if canPedBeUsed(RandomNPC, false) then
                                if GetWeapontypeGroup(GetSelectedPedWeapon(ped)) == -728555052 then
                                    TriggerServerEvent("esx_outlawalert:server:NewAlert",
                                        'police', nil, "Suspeito Armado",
                                        GetLabelFromWeapon(GetSelectedPedWeapon(ped)), "remove_red_eye", "gps_fixed", 1,
    ...
    while true do
        ...
        if DoesEntityExist(ped) and not IsEntityDead(ped) then
            if GetSelectedPedWeapon(ped) ~= GetHashKey("WEAPON_UNARMED") then
                                ...
                                if GetDistanceBetweenCoords(playerCoords, RandomNPcoords, true) < 100.0 then
                                    ...
                                    if canPedBeUsed(RandomNPC, false) and not IsPedCurrentWeaponSilenced(ped) then
                                        TriggerServerEvent("esx_outlawalert:gunshotInProgress",
                                            'police', nil, "Relato de tiros", label, "volume_up", "gps_fixed", 1,
                ...
                if FoundEntity and LastEntity ~= AimedEntity and IsPedInAnyVehicle(AimedEntity, false) then
                    ...
                    if math.random() >= 0.25 then
                        if not IsPedAPlayer(AimedEntity) and
                            PlayerData.job.name ~= 'police' then
                            ...
                            TriggerServerEvent("esx_outlawalert:carJackInProgress",
                                'police', nil, "Viatura Roubada", vehicleLabel .. " - " .. primary, "palette",
                                "gps_fixed", 1,
                            ...
                            TaskPlayAnim(AimedEntity, "random@mugging3", "handsup_standing_base", 8.0, -8, 0.01, 49, 0, 0
                                , 0, 0)
            ...
            if IsPedJacking(ped) == 1 or IsPedTryingToEnterALockedVehicle(ped) then
                waitWeap = 20
                TriggerEvent("esx_status:add", "stress", 30000)
                if not called then
                    --    callCops()
                    called = true
                else
    ...
    Citizen.CreateThread(function()
        ...
        if rand <= 100 then
            ...
            if vehicle then
                ...
                local playerCoords = GetEntityCoords(ped)
                TriggerServerEvent("esx_outlawalert:carJackInProgress",
                    'police', nil, "Viatura Roubada", vehicleLabel .. " - " .. primary, "palette", "gps_fixed", 1,
...
RegisterNetEvent('esx_outlawalert:PlaySoundPanic')
AddEventHandler('esx_outlawalert:PlaySoundPanic', function()
...
RegisterCommand('panic', function(src, args, raw)
    if PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' then
        ...
        TriggerServerEvent("esx_outlawalert:sendPanic", 'police', 'ambulance', nil, "Botão Pânico", myName,
            "person", "gps_fixed", 1, coord.x, coord.y, coord.z, 480, 75, "10-99")
...
RegisterCommand('911', function(src, args, raw)
    if args[1] ~= nil then
        ...
        if #(myName .. " - " .. raw:sub(4)) <= 50 then
            ...
            TriggerServerEvent("esx_outlawalert:send911", 'police', 'ambulance', nil, "Central 911", message,
                "person", "gps_fixed", 1, coord.x, coord.y, coord.z, 480, 75, "911")
        else
            ESX.ShowNotification("SERVIÇOS DE EMERGÊNCIA", "Insere um motivo mais curto", 2000, 'error')
        end
    else
        ESX.ShowNotification("SERVIÇOS DE EMERGÊNCIA", "Insere um motivo", 2000, 'error')
    end
end)
...
RegisterCommand('mec', function(src, args, raw)
    if args[1] ~= nil then
        ...
        if #(myName .. " - " .. raw:sub(4)) <= 60 then
            message = myName .. " - " .. raw:sub(4)
            TriggerServerEvent("esx_outlawalert:sendmecanicos", 'mechanic', nil, "Central Seguros", message,
                "person", "gps_fixed", 1, coord.x, coord.y, coord.z, 106, 75, "Benny´s")
            ESX.ShowNotification("SERVIÇOS DE EMERGÊNCIA", "Pedido de ajuda enviado com sucesso", 2000, 'success')
        else
            ESX.ShowNotification("SERVIÇOS DE EMERGÊNCIA", "Insere um motivo mais curto", 2000, 'error')
        end
    else
...
RegisterNetEvent('esx_outlawalert:setMyName')
AddEventHandler('esx_outlawalert:setMyName', function(name)
...
RegisterNetEvent('esx_outlawalert:exportName')
AddEventHandler('esx_outlawalert:exportName', function(cb)
    cb(myName)
end)
...
RegisterNetEvent('esx_outlawalert:NewAlert')
AddEventHandler('esx_outlawalert:NewAlert',
    function(job, title, desc, emote, emote2, number, x, y, z, blipID, color, code)
...
RegisterCommand('notimenu', function(source)
    TriggerEvent('esx_outlaw:openMenu')
end)

RegisterNetEvent('esx_outlaw:openMenu')
AddEventHandler('esx_outlaw:openMenu', function()
