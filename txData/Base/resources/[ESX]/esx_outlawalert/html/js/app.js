$(document).ready(function() {
    ...
    document.onkeydown = function (e) {
        if (e.which == 27 || e.which == 9) { // Escape key 
            $(".notification").slideDown("fast", function() {
                $.post('http://esx_outlawalert/close', JSON.stringify({}));
                $('.notification').css('display', 'none');
                show =  false
            });
        }
    ...
    function DeleteAlerts(event){
        $.each(event.data.notifications, function (index, notification) {
            $.post('http://esx_outlawalert/deleteAlert', JSON.stringify({id:indexId}));
    ...
    $(document).on('click', '.loc', function () {
        $.post('http://esx_outlawalert/createBlip', JSON.stringify({id:$(this).data('id')}));
    });

    $(document).on('click', '.delete a', function () {
        $(this).parent().parent().remove();
        
        $.post('http://esx_outlawalert/deleteAlert', JSON.stringify({id:$(this).parent().data('id')}));
