	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				xPlayer.showNotification("SOCIEDADE", _U('have_withdrawn', ESX.Math.GroupDigits(amount)), 2000, 'success')
			else
				xPlayer.showNotification("SOCIEDADE", _U('invalid_amount'), 2000, "error")
			end
		end)
	else
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				xPlayer.showNotification("SOCIEDADE", _U('have_deposited', ESX.Math.GroupDigits(amount)), 2000, 'success')
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			function(rowsChanged)
				xPlayer.showNotification("FATURAS", _U('you_have', ESX.Math.GroupDigits(amount)), 2000, 'success')
			end)
		else
			xPlayer.showNotification("FATURAS", _U('invalid_amount'), 2000, "error")
		end
	else
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				xTarget.showNotification("SOCIEDADE", _U('you_have_been_hired', job), 2000, 'success')
			elseif type == 'promote' then
				xTarget.showNotification("SOCIEDADE", _U('you_have_been_promoted'), 2000, 'success')
			elseif type == 'fire' then
				xTarget.showNotification("SOCIEDADE", _U('you_have_been_fired', xTarget.getJob().label), 2000, 'success')
			end
...
function WashMoneyCRON(d, h, m)
	MySQL.query('SELECT * FROM society_moneywash', function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				xPlayer.showNotification("LAVAGEM", _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)), 2000, 'success')
			end
