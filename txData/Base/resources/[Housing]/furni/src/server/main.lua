...
SqlReady = function()
  ...
  SqlFetch("SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA=@dbName AND TABLE_NAME=@tabName",
    { ['@dbName'] = dbName, ['@tabName'] = dbTable }, function(retData)
    ...
    if truePrice and furni.canPlayerAfford(source, truePrice) then
      SqlFetch("SELECT * FROM " .. (Config and Config.SaveToTable or "allhousing"), {}, function(retData)
        for _, data in pairs(retData) do
          ...
          if (
              math.floor(tonumber(houseData.Entry.x)) == math.floor(tonumber(entry.x)) and
                  math.floor(tonumber(houseData.Entry.y)) == math.floor(tonumber(entry.y)) and
                  math.floor(tonumber(houseData.Entry.z)) == math.floor(tonumber(entry.z))) then
            ...
            SqlExecute("UPDATE " ..
              (Config and Config.SaveToTable or "allhousing") .. " SET furniture=@furniture WHERE id=@id",
              { ['@furniture'] = json.encode(jTab), ['@id'] = data.id })
        ...
        if (
          ...
          if closest and closestDist and closestDist < 1.0 then
            ...
            if truePrice then
              ...
              furni.givePlayerMoney(source,
                math.floor(truePrice * (Config.ResaleValue and Config.ResaleValue / 100.0 or 0.5)))
              SqlExecute("UPDATE " ..
                (Config and Config.SaveToTable or "allhousing") .. " SET furniture=@furniture WHERE id=@id",
                { ['@furniture'] = json.encode(jTab), ['@id'] = data.id })
              TriggerEvent("Allhousing:SetFurni", houseData, furniture)
    ...
    SqlFetch("SELECT * FROM " .. (Config and Config.SaveToTable or "allhousing"), {}, function(retData)
      for _, data in pairs(retData) do
        ...
        if (
          ...
          local jTab = {}
          for k, v in pairs(furniture) do
            if math.floor(v.pos.x + houseData.Entry.x) == math.floor(lastObject.pos.x) and
                math.floor(v.pos.y + houseData.Entry.y) == math.floor(lastObject.pos.y) and
                math.floor(v.pos.z + houseData.Entry.z) == math.floor(lastObject.pos.z) and v.model == itemData.object then
