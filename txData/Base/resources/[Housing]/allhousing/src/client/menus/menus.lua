...
OpenInventory = function()
  if Config.UsingESX then
    if not Config.UsingDiscInventory then
      ...
      --end,InsideHouse)
      ESX.TriggerServerCallback("Allhousing:getPlayerIdentifier", function(identifier)
        local other = {}
        other.maxweight = 500000
        other.slots = 500
        TriggerServerEvent("inventory:server:OpenInventory", "stash", "Casa_" .. InsideHouse.Id, other)
        TriggerEvent("inventory:client:SetCurrentStash", "Casa_" .. InsideHouse.Id)
        --TriggerEvent('inventory:openStorage', "Casa", "casa-"..InsideHouse.Id, 2500, 1000, false)
      end)
    else
...
SpawnHouse = function(d)
  ...
  local shell = CreateObject(hash, d.Entry.x + Config.SpawnOffset.x, d.Entry.y + Config.SpawnOffset.y,
    d.Entry.z - 30.0 + Config.SpawnOffset.z, false, false)
...
TeleportInside = function(d, v)
  local exitOffset = vector4(ShellOffsets[d.Shell]["exit"].x - Config.SpawnOffset.x,
    ShellOffsets[d.Shell]["exit"].y - Config.SpawnOffset.y, ShellOffsets[d.Shell]["exit"].z - Config.SpawnOffset.z,
    ShellOffsets[d.Shell]["exit"].w)
...
SellHouse = function(d)
  ...
  if not d.MortgageOwed or d.MortgageOwed <= 0 then
    exports["input"]:Open(Labels["SetSalePrice"], (Config.UsingESX and Config.UsingESXMenu and "ESX" or "Native"),
      function(data)
...
RepayMortgage = function(d)
  ...
  if GetPlayerCash() > min_repayment or GetPlayerBank() > min_repayment then
    exports["input"]:Open(string.format("Min: $%i", min_repayment),
      (Config.UsingESX and Config.UsingESXMenu and "ESX" or "Native"), function(res)
