...
RegisterServerEvent('housing:removeOutfit')
AddEventHandler('housing:removeOutfit', function(label)
  local xPlayer = ESX.GetPlayerFromId(source)
  TriggerEvent('esx_datastore:getDataStore', 'property', xPlayer.identifier, function(store)
...
VehicleSpawned = function(plate)
  MySQL.Async.execute("UPDATE owned_vehicles SET `stored`=false,`storedhouse`=@storedhouse WHERE `plate`=@plate",
    { ['@storedhouse'] = 0, ['@plate'] = plate })
end

VehicleStored = function(id, plate, props)
  MySQL.Async.execute("UPDATE owned_vehicles SET `stored`=false,`storedhouse`=@storedhouse WHERE `plate`=@plate",
    { ['@storedhouse'] = id, ['@plate'] = plate })
end

GetInventory = function(source, cb, house)
  local v = Houses[house.Id]
  cb({ cash = v.Inventory.Cash, blackMoney = v.Inventory.DirtyMoney, items = v.Inventory.Items,
    weapons = v.Inventory.Weapons })
end

ESX.RegisterServerCallback('Allhousing:getPlayerIdentifier', function(source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
...
PurchaseHouse = function(house)
  ...
  if not v.Owned then
    ...
    if afford then
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          if targetPlayer then
            ...
            NotifyPlayer(targetSource,
              string.format(Labels["SolddHouse"], v.Price, (v.Price ~= salePrice and Labels["HouseEarning"] or ".")))
          else
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            local societyAmount = (
                Config.CreationJobs[v.ResaleJob].society and math.floor(v.Price * ((100 - v.ResalePercent) / 100)))
      ...
      SqlExecute("UPDATE allhousing SET owner=@owner,ownername=@ownername,resalejob=@resalejob,owned=1,housekeys=@housekeys WHERE id=@id"
        ,
        { ['@owner'] = GetPlayerIdentifier(_source), ['@ownername'] = v.OwnerName, ['@housekeys'] = json.encode({}),
          ['@resalejob'] = "", ['@id'] = v.Id })
    end
  end
end
...
SellHouse = function(house, price)
  ...
  if v.Entry.x == entry.x and v.Entry.y == entry.y and v.Entry.z == entry.z then
    if v.Owned and v.Owner == GetPlayerIdentifier(_source) then
            ...
            if v and v.Furniture and type(v.Furniture) == "table" then
              ...
              if count and count > 0 then
                _print("[Sale]",
                  ...
                  _source ..
                  " (" ..
                  GetPlayerIdentifier(_source) ..
                  "/" ..
                  GetPlayerName(_source) ..
                  ") to refund " ..
                  (count and count > 1 and count .. " pieces" or tostring(count) .. " piece") .. " of furniture.")
      ...
      SqlExecute("UPDATE " ..
        ((Config and Config.AllhousingTable) or "allhousing") ..
        " SET owned=0,price=@price,resalepercent=100,resalejob=@resalejob,furniture=@furniture WHERE id=@id",
        { ['@furniture'] = json.encode(furniTab), ['@resalejob'] = "", ['@price'] = price, ['@id'] = v.Id })
      return
    end
  end
end
...
MortgageHouse = function(house)
  ...
  if not v.Owned then
    ...
    if afford then
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          if targetPlayer then
            ...
            AddPlayerBank(targetSource, salePrice)
            NotifyPlayer(targetSource,
              "Your house was mortgaged for $" ..
              price .. (price ~= salePrice and ", you earnt $" .. salePrice .. " from the sale." or "."))
          else
            AddOfflineBank(v.Owner, salePrice)
          end
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            local societyAmount = (
                Config.CreationJobs[v.ResaleJob].society and math.floor(price * ((100 - v.ResalePercent) / 100)))
            local societyAccount = Config.CreationJobs[v.ResaleJob].account
            if societyAmount and societyAccount then
              AddSocietyMoney(societyAccount, societyAmount)
            end
      ...
      TriggerClientEvent("Allhousing:SyncHouse", -1, v)
      SqlExecute("UPDATE allhousing SET owner=@owner,ownername=@ownername,resalejob=@resalejob,owned=1,housekeys=@housekeys,mortgage_owed=@mortgage_owed,last_repayment=@last_repayment WHERE id=@id"
        ,
        { ['@owner'] = GetPlayerIdentifier(_source), ['@ownername'] = v.OwnerName, ['@housekeys'] = json.encode({}),
          ['@resalejob'] = "", ['@mortgage_owed'] = v.MortgageOwed, ['@last_repayment'] = v.LastRepayment, ['@id'] = v.Id })
    end
  end
end
...
RevokeTenancy = function(house)
  ...
  if house.Owned and house.MortgageOwed >= 0 then
    ...
    TriggerClientEvent("Allhousing:SyncHouse", -1, house)
    SqlExecute("UPDATE allhousing SET owner=@owner,ownername=@ownername,resalejob=@resalejob,owned=0,housekeys=@housekeys,mortgage_owed=0,last_repayment=0 WHERE id=@id"
      ,
      { ['@owner'] = house.Owner, ['@ownername'] = house.OwnerName, ['@housekeys'] = json.encode({}),
        ['@resalejob'] = house.ResaleJob, ['@id'] = house.Id })
  end
end
...
SqlCheck = function()
  ...
  if Houses and type(Houses) == "table" then
    for k1, v1 in pairs(Houses) do
      ...
      for k2, v2 in pairs(curInDb) do
        ...
        if (
            math.floor(tonumber(v1.Entry.x)) == math.floor(tonumber(entry.x)) and
                math.floor(tonumber(v1.Entry.y)) == math.floor(tonumber(entry.y)) and
                math.floor(tonumber(v1.Entry.z)) == math.floor(tonumber(entry.z))) then
          foundMatch = k2
          break
        end
      end
      if not foundMatch then
        ...
        newHouses[curInDb[foundMatch].id].Garage = (
            curInDb[foundMatch].garage:len() > 5 and table.tovec(json.decode(curInDb[foundMatch].garage)) or false)
        newHouses[curInDb[foundMatch].id].Furniture = json.decode(curInDb[foundMatch].furniture)
        newHouses[curInDb[foundMatch].id].Shell = (
            curInDb[foundMatch].shell ~= "0" and curInDb[foundMatch].shell or false)
        newHouses[curInDb[foundMatch].id].Interior = (
            curInDb[foundMatch].Interior ~= "0" and tonumber(curInDb[foundMatch].Interior) or false)
        ...
        newHouses[curInDb[foundMatch].id].Wardrobe = (
            curInDb[foundMatch].wardrobe:len() > 5 and table.tovec(json.decode(curInDb[foundMatch].wardrobe)) or false)
        newHouses[curInDb[foundMatch].id].Inventory = (
            curInDb[foundMatch].inventory:len() > 5 and json.decode(curInDb[foundMatch].inventory) or false)
        newHouses[curInDb[foundMatch].id].InventoryLocation = (
            curInDb[foundMatch].inventorylocation:len() > 5 and
                table.tovec(json.decode(curInDb[foundMatch].inventorylocation)) or false)
    ...
    for k1, v1 in pairs(curInDb) do
      ...
      if entry ~= nil then
        for k2, v2 in pairs(newHouses) do
          if (
              math.floor(tonumber(v2.Entry.x)) == math.floor(tonumber(entry.x)) and
                  math.floor(tonumber(v2.Entry.y)) == math.floor(tonumber(entry.y)) and
                  math.floor(tonumber(v2.Entry.z)) == math.floor(tonumber(entry.z))) then
            foundMatch = k2
            break
          end
        end
        if not foundMatch then
          newHouses[v1.id] = {
            ...
            InventoryLocation = (
                v1.inventorylocation:len() > 5 and table.tovec(json.decode(v1.inventorylocation)) or false),
  ...
  if addToDb and type(addToDb) == "table" then
    for k, v in pairs(addToDb) do
      SqlExecute("INSERT INTO " ..
        (Config.AllhousingTable or "allhousing") ..
...
UpgradeHouse = function(house, shell)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    ...
    if afford then
      v.Shell = shell
      SyncHouse(v)
      SqlExecute("UPDATE " .. ((Config and Config.AllhousingTable) or "allhousing") .. " SET shell=@shell WHERE id=@id",
        { ['@shell'] = shell, ['@id'] = v.Id })
...
GiveKeys = function(house, target)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    ...
    SqlExecute("UPDATE " ..
      ((Config and Config.AllhousingTable) or "allhousing") .. " SET housekeys=@housekeys WHERE id=@id",
      { ['@housekeys'] = json.encode(v.HouseKeys), ['@id'] = v.Id })
  end
end
...
SetWardrobe = function(house, wardrobe)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    v.Wardrobe = wardrobe
    SyncHouse(v)
    SqlExecute("UPDATE " .. ((Config and Config.AllhousingTable) or "allhousing") ..
      " SET wardrobe=@wardrobe WHERE id=@id",
      { ['@wardrobe'] = json.encode({ x = wardrobe.x, y = wardrobe.y, z = wardrobe.z }), ['@id'] = v.Id })
...
SetInventory = function(house, inventoryLocation)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    ...
    SyncHouse(v)
    SqlExecute("UPDATE " ..
      ((Config and Config.AllhousingTable) or "allhousing") .. " SET inventorylocation=@inventorylocation WHERE id=@id",
      { ['@inventorylocation'] = json.encode({ x = inventoryLocation.x, y = inventoryLocation.y, z = inventoryLocation.z }),
        ['@id'] = v.Id })
...
GetItem = function(house, itemType, itemName, itemCount)
  ...
  if itemType == 'item_money' then
    ...
    exports["Pack"]:LogToDiscord(_source,
      "https://discord.com/api/webhooks/1043595224850628698/9oEFOTWwLj1XPpy61M-viuBQrieLWiq8cg3dVUb_5xahJ_iFzqlw2JpAVr5eyqXzp4Y-"
      , "Dinheiro retirado casas",
      "O jogador " .. GetPlayerName(_source) .. " pegou " .. itemCount .. " de dinheiro do cofre da casa.")
    v.Inventory.Cash = math.floor(v.Inventory.Cash - itemCount)
    --SyncHouse(v)
    SqlExecute("UPDATE " ..
      ((Config and Config.AllhousingTable) or "allhousing") .. " SET inventory=@inventory WHERE id=@id",
      { ['@inventory'] = json.encode(v.Inventory), ['@id'] = v.Id })
        ...
        if item.name == itemName then
          ...
          if xPlayer.canCarryItem(itemName, itemCount) then
            ...
            exports["Pack"]:LogToDiscord(_source,
              "https://discord.com/api/webhooks/1043595224850628698/9oEFOTWwLj1XPpy61M-viuBQrieLWiq8cg3dVUb_5xahJ_iFzqlw2JpAVr5eyqXzp4Y-"
              , "Dinheiro retirado casas",
              "O jogador " .. GetPlayerName(_source) .. " pegou " .. itemCount .. " de " ..
              itemName .. " do cofre da casa.")
            --SyncHouse(v)
            SqlExecute("UPDATE " ..
              ((Config and Config.AllhousingTable) or "allhousing") .. " SET inventory=@inventory WHERE id=@id",
              { ['@inventory'] = json.encode(v.Inventory), ['@id'] = v.Id })
...
PutItem = function(house, itemType, itemName, itemCount)
  ...
  if itemType == 'item_money' then
    ...
    exports["Pack"]:LogToDiscord(_source,
      "https://discord.com/api/webhooks/1043595224850628698/9oEFOTWwLj1XPpy61M-viuBQrieLWiq8cg3dVUb_5xahJ_iFzqlw2JpAVr5eyqXzp4Y-"
      , "Dinheiro retirado casas",
      "O jogador " .. GetPlayerName(_source) .. " colocou " .. itemCount .. " de dinheiro no cofre da casa.")
    --SyncHouse(v)
    SqlExecute("UPDATE " ..
      ((Config and Config.AllhousingTable) or "allhousing") .. " SET inventory=@inventory WHERE id=@id",
      { ['@inventory'] = json.encode(v.Inventory), ['@id'] = v.Id })
      ...
      for _, item in pairs(v.Inventory.Items) do
        if item.name == itemName then
          ...
          exports["Pack"]:LogToDiscord(_source,
            ...
            , "Dinheiro retirado casas",
            "O jogador " ..
            GetPlayerName(_source) .. " colocou " .. itemCount .. " de " .. itemName .. " no cofre da casa.")
          --SyncHouse(v)
          SqlExecute("UPDATE " ..
            ((Config and Config.AllhousingTable) or "allhousing") .. " SET inventory=@inventory WHERE id=@id",
            { ['@inventory'] = json.encode(v.Inventory), ['@id'] = v.Id })
...
GetMortgageInfo = function(source, house)
  return { MortgageOwed = Houses[house.Id].MortgageOwed,
    LastRepayment = GetLastRepayment(os.time(), Houses[house.Id].LastRepayment) }
end
...
AddEventHandler("Allhousing:FullDeleteHouse", function(house)
  ...
  if xPlayer.getGroup() == 'admin' or xPlayer.getGroup() == 'superadmin' then
    ...
    Houses[house.Id] = nil
    xPlayer.showNotification("CASAS", 'Casa apagada com sucesso!', 2000, 'success')
  else
    xPlayer.showNotification("CASAS", 'Não tens permissões para fazer isto.', 2000, 'error')
  end
end)
