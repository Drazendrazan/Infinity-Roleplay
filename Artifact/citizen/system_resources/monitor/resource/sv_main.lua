...
local function handleKickEvent(eventData)
    DropPlayer(eventData.target, '[Infinity RP] ' .. eventData.reason)
end
            ...
            for _, searchIdentifier in pairs(eventData.targetIds) do
                ...
                for _, playerIdentifier in pairs(identifiers) do
                    if searchIdentifier == playerIdentifier then
                        ...
                        DropPlayer(playerID, '[Infinity RP] ' .. eventData.kickMessage)
                        found = true
                        break
                    end
                end
...
local function handleShutdownEvent(eventData)
    ...
    for _, serverID in pairs(players) do
        DropPlayer(serverID, '[Infinity RP] ' .. eventData.message)
    end
end
...
function handleConnections(name, setKickReason, d)
    ...
    if rejectAllConnections then
        CancelEvent()
        setKickReason("[Infinity RP] Server is shutting down, try again in a few seconds.")
        return
    end
    ...
    if GetConvar("txAdmin-checkPlayerJoin", "invalid") == "true" then
        ...
        if #exData.playerIds <= 1 then
            d.done("[Infinity RP] You do not have at least 1 valid identifier. If you own this server, make sure sv_lan is disabled in your server.cfg")
            return
        end

        --Attempt to validate the user
        d.update("[Infinity RP] Checking banlist/whitelist... (0/10)")
        CreateThread(function()
            local attempts = 0
            local isDone = false;
            --Do 10 attempts
            while isDone == false and attempts < 10 do
                attempts = attempts + 1
                d.update("[Infinity RP] Checking banlist/whitelist... ("..attempts.."/10)")
                PerformHttpRequest(url, function(httpCode, rawData, resultHeaders)
                    -- Validating response
                    local respStr = tostring(rawData)
                    if httpCode ~= 200 then
                        logError("[Infinity RP] Checking banlist/whitelist failed with code "..httpCode.." and message: "..respStr)
                    end
                    local respObj = json.decode(respStr)
                    if not respObj or type(respObj.allow) ~= "boolean" then
                        logError("[Infinity RP] Checking banlist/whitelist failed with invalid response: "..respStr)
                    end
                    ...
                    if respObj.allow == true then
                        ...
                        if not isDone then
                            local reason = respObj.reason or "[Infinity RP] no reason provided"
                            d.done("\n"..reason)
                            isDone = true
                        end
                    end
            ...
            if not isDone then
                d.done('[Infinity RP] Failed to validate your banlist/whitelist status. Try again later.')
                isDone = true
            end
        end)
